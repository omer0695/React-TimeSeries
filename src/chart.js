...
class Chart extends React.Component {
    // requesting cones data from API instead of hard coded json
    constructor(props) {
        super(props);
        this.state = {
            cones: []
    ...
    drawChart() {
        const {riskLevel, initialSum, monthlySum } = this.props;
        const {mu, sigma} = this.state.cones.filter(cone => cone.riskLevel == riskLevel)[0];
        const fee = 0.01;
